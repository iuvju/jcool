...
URLParser.prototype.getURL = function() {
	var url = '';
	for(var f in this._fields) if (typeof this._values[f] != 'undefined')  
    {  
        url += this._values[f]; 
    }  
	console.log(url);
...
var act = function(data){
	css_link.forEach(function(item){
		var url = new URLParser(item.href);
		var host = url.getHost();
		var new_host = checkStat(data, host), new_url;
		if(new_host){
			url.setValue('Host', new_host);
			url.getURL();
		}
